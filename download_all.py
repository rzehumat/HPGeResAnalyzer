...
def download_isotopes_list(Z):
    ...
    urllib.request.urlretrieve(
        request_url, f"downloads/find_isotopes/z_{Z}.html")
...
def no_gammas_warning(A, Z, str_A, element):
    print(colored(
        f"Seems like there are no gamma-lines known for isotope {A}{element}.",
        'yellow'))
    print(colored("Check yellow pages for reference.", 'yellow'))
    print(colored(
        f"http://nucleardata.nuclear.lu.se/toi/nuclide.asp?iZA={Z}{str_A}",
        'yellow'))
...
def download_all_isotopes(Z):
    ...
    for A in A_lst:
        ...
        urllib.request.urlretrieve(
            url,
            f"downloads/isotopes_html/{A}_{abbr}_{Z}.html")
...
def extract_Igamma(A, element, Z):
    ...
    try:
        ...
        if any(x in gammas_table.find("font")
                .get_text(strip=True) for x in ["Betas", "X-rays"]):
            A, str_A = process_A(A)
    ...
    for row in gammas_rows:
        ...
        if "<i>" in str(i_cell):
            ...
            sigm_ig_val = (float(sigm_ig_val)
                           * pow(10, ig_decimals.as_tuple().exponent))
            ig_val = float(ig_val)
        ...
        if "<i>" in str(e_cell):
            ...
            sigm_e_val = (float(sigm_e_val)
                          * pow(10, e_decimals.as_tuple().exponent))
            e_val = float(e_val)
    ...
    print(
        f"Ig extracted from file 'downloads/{A}{element}.html'"
        f" into '{df_name}'.")
    return 0
...
def extract_element(Z):
    ...
    element_files = [
        f for f in html_lst if str(Z) == f.split('_')[-1].split('.')[0]]
...
def extract_info(A, element, Z):
    ...
    if "Literature cut-off date" in columns:
        info_df["Literature cut-off date"] = pd.to_datetime(
            info_df["Literature cut-off date"])
    ...
    if info_df["Half life"][0] == "stable" or info_df["Half life"][0] == "":
        ...
        if hl_unit not in list(TIME_CONVERSION.keys()):
            ...
            if "sigm_Half life" in columns:
                if "+" in str(info_df["sigm_Half life"][0]):
                    info_df["sigm_Half life"][0] = (
                        (str(info_df["sigm_Half life"][0])
                         .split('+')[1]).split('-')[0]).strip()

                info_df["sigm_Half-life [s]"] = (
                    int(info_df["sigm_Half life"][0])
                    * pow(10, d.as_tuple().exponent)
                    * TIME_CONVERSION[hl_unit])
